...
class DbQuerier():
    ...
    def __init__(self, gps:GPS, choose_specific:bool=True) -> None:
        self.gps = gps
        self.new_data_day_soc_df = pd.DataFrame()
        self.new_data_day_v_df = pd.DataFrame()

        self.save_directory_soc = os.path.dirname(os.path.abspath(__file__))
        self.save_directory_velocity = os.path.dirname(os.path.abspath(__file__))
        ...
        if choose_specific:
            ...
            chosen_directory = filedialog.askdirectory(title='Select the Solar_car Folder')
            ...
            dir = 'G:\\Shared drives\\AlphaCentauri\\SolarCar_22 23\\6. Strategy & Simulation\\ss_online_data\\Solar_car'
    ...
    def save_data2folder(self) -> None:
        ...
        print(f'Data saved to {os.path.join(self.save_directory_velocity, self.CSV_VELOCITY)}')